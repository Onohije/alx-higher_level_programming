...
class TestSquare_stdout(unittest.TestCase):
    ...
    def capture_stdout(sq, method):
        """Captures and returns the text that is printed to stdout when
            a specific method is called on a given Square object.

          Args:

           sq (Square): The Square object to be printed to stdout.
           method (str): The method to be executed on sq.
          Returns:
            The text that is printed to stdout when calling the 
            specified method on the Square object, sq.
        """
        capture = io.StringIO()
        sys.stdout = capture
        if method == "print":
